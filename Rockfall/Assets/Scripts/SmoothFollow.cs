...
public class SmoothFollow : MonoBehaviour
{
    ...
    //Насклько замедляются изменения в повороте и высоте
    ...
    private void LateUpdate()
    {
        ...
        //Продолжить выполнять замедленный поворот вокруг оси у
        currentRotationAngle = Mathf.LerpAngle(currentRotationAngle, wantedRotationAngle, rotationDamping * Time.deltaTime);

        //Продолжить постепенно корректировать высоты над целью 
        currentHeight = Mathf.Lerp(currentHeight, wantedHeight, heightDamping * Time.deltaTime);

        //Преобразовать угол в поровот 
        var currentRotation = Quaternion.Euler(0, currentRotationAngle, 0);

        //Установить местоположение камеры в плоскости x-z на расстоянии в "distance" метрах от цели
        ...
        //Сориентировать объектив камеры в сторону, куда направляется целевой объект
